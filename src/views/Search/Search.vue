<template>
  <div>
    ...
    <div
      class="shopContain"
      :class="{ noloadtips: noload === 1, loadtips: noload === 0 && isShowLoad === 1}"
      @scroll="loading"
      ref="shopContain"
    >
      <shop-item
        v-for="item of shop.list"
        :key="item.shop_id"
        :shopInfo="item"
      />
    </div>
...
export default {
  ...
  updated() {
    // 是否显示底部tip
    if (this.$refs.shopContain.offsetHeight > 517) {
      this.isShowLoad = 1;
    } else {
      this.isShowLoad = 0;
    }
  ...
  methods: {
    ...
    loading(event) {
      ...
      if (
        scrollele.offsetHeight + scrollele.scrollTop >
          scrollele.scrollHeight - 30 &&
        this.isload === 0
      ) {
        this.isload = 1;
        if (this.noload === 0) {
          this.shop.page++;
          shopList(this.shop.page, this.shop.cateName).then((val) => {
            //   console.log(val)
            if (val.length === 0) {
              this.shop.page--;
              this.noload = 1;
